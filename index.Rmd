...
```
```{r download-staff-photos, include = FALSE}
...
```{r keywordcloud, fig.height = 7.5, fig.width = 15}
...
photos <- list.files("images/") %>% 
  str_subset("^photo")
who <- function(name, pos = '') {
  pics <- map_chr(name, ~{
      apic <- str_subset(photos, .x)
      glue("<img class='circle' src='images/{apic}'>") %>% 
        as.character()
    }) %>% 
    paste(collapse = "")
...
<a href="https://www.monash.edu/business/ebs/research/showcase/new-methods-for-modelling-complex-trends-in-climate-and-energy-time-series">
.box[
.box-up[
<img src="https://www.monash.edu/__data/assets/image/0011/2199458/Turbine.jpg" width="100%">
]
.box-bottom[
Climate & energy econometrics
]
]
</a>
]
...
walk(str_extract(logos, "[a-z]+.png"), ~print(print_logo(.x)))
```

.fr[
Non-Uniform Monash Business Analytics Team (.monash-blue[**NUMBAT**])
...
# Tools with human-centered designs
...
.fr[
`r print_logo("edibble.png", who = c("tanaka"), pos = 'right:200%;top:200%;')` `r print_logo("deggust.png", who = c("tanaka"), pos = 'right:200%;top:200%;')`
...
walk(str_subset(logo_other, "nullabor"), ~print(glue("<img src='images/{.x}' width='120px'>")))
```
.fr[
## Spatial visualisation

```{r, echo = FALSE, results = "asis"}
walk(str_subset(logo_other, "sugarbag"), ~print(glue("<img src='images/{.x}' width='120px'>")))
```

]

## High-dimensinal data visualisation

```{r, echo = FALSE, results = "asis"}
walk(str_subset(logo_other, "(spinifex|tourr)"), ~print(glue("<img src='images/{.x}' width='120px'>")))
```
...
```{r, echo = FALSE, results = "asis"}
walk(str_subset(logo_other, "(lookout|stray|odd)"), ~print(glue("<img src='images/{.x}' width='120px'>")))
```
